...
import com.aht.api.evaluator.EuclideanLength;
 ...
 */
public class ItemPredictorCalculatedSimilitude implements ItemPredictor {
    public double predictValue(User user, Item item) {
        //TODO: Create item prediction for general implementation.
        /* Get user events
        *  For Each: Get event value
        *  Search for most similar items in events
        *  Analyse and return value for current item
        */
        EuclideanLength euclideanLength = new EuclideanLength();
        List<Event> events = user.getModelEvents();
        LinkedList<Wrapper> wrappers = new LinkedList<>();
        for(Event event : events){
            double value = event.getModelValue();
            double distance = euclideanLength.getEvaluationForItems(event.getModelItem(), item);
            wrappers.add(new Wrapper(event.getModelItem(),value, distance));
        }
